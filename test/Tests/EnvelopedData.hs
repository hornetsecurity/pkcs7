...
instance Monad m => Serial m Originator where
    series = decDepth $
        Originator <$> elements [ Nothing, Just [], Just [ example ] ]
...
instance Monad m => Serial m KeyTransport where
    series = decDepth $
        KeyTransport <$> pure example <~> series <~> series <~> series
...
instance Monad m => Serial m KEK where
    series = decDepth $
        KEK <$> pure example <~> pure example <~> series <~> series
...
instance Monad m => Serial m Password where
    series = decDepth $
        Password <$> pure example <~> series <~> series <~> series
...
sampleEnvelopedData =
    BS.concat [ "MIICBQYJKoZIhvcNAQcDoIIB9jCCAfICAQAxggGpMIIBpQIBADCBjDB/MQswCQYDVQQGEwJE"
...
testEnvelopedData =
    testGroup "Data.Pkcs7.EnvelopedData"
