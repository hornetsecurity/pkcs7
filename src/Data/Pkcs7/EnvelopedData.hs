...
kaaTable = [ (KeyAgreementESDH, oidESDH)
           , (KeyAgreementSSDH, oidSSDH)
           ]
...
kwaTable = [ (KeyWrapDES3, oidDES3Wrap)
           , (KeyWrapRC2, oidRC2Wrap)
           ]
...
instance ASN1Structure Originator where
    toASN1Fields Originator{..} = runPrintASN1State printer
        where printer = putImplicitMaybe 0 (SetOf <$> originatorCerts)
                        <> putImplicitMaybe 1 (SetOf <$> originatorCrls)
    fromASN1Fields = runParseASN1State parser
        where parser = Originator <$> (fmap unSetOf <$> getImplicitMaybe 0)
...
data KeyTransportRecipientIdentifier = KeyTransportRecipientIssuerAndSerial IssuerAndSerial
...
instance ASN1Object KeyTransportRecipientIdentifier where
    ...
    fromASN1 = runParseASN1State parser
        where parser = (fmap KeyTransportRecipientSubjectKeyIdentifier <$> getExplicitMaybe 0)
                       `orChoiceDefault`
...
data OriginatorPublicKey = OriginatorPublicKey { originatorPublicKeyAlgorithm :: SignatureAlgorithmIdentifier
...
instance ASN1Structure OriginatorPublicKey where
    toASN1Fields OriginatorPublicKey{..} = runPrintASN1State printer
        where printer = putObject originatorPublicKeyAlgorithm
                        <> putBitString originatorPublicKey
    fromASN1Fields = runParseASN1State parser
        where parser = OriginatorPublicKey <$> getObject
                                           <*> getBitString
...
data OriginatorIdentifierOrKey = OriginatorIssuerAndSerial IssuerAndSerial
...
instance ASN1Object OriginatorIdentifierOrKey where
    ...
    fromASN1 = runParseASN1State parser
        where parser = (fmap OriginatorKey <$> getExplicitMaybe 1)
                       `orChoice`
...
instance ASN1Object UserKeyingMaterial where
    ...
    fromASN1 = runParseASN1State parser
        where parser = UserKeyingMaterial <$> getOctetString

data KeyAgreementRecipientIdentifier = KeyAgreementRecipientIdentifierIssuerAndSerial IssuerAndSerial
...
instance ASN1Object KeyAgreementRecipientIdentifier where
    ...
    fromASN1 = runParseASN1State parser
        where parser = (fmap KeyAgreementRecipientIdentifierKeyIdentifier <$> getImplicitMaybe 0)
                       `orChoiceDefault`
                       (KeyAgreementRecipientIdentifierIssuerAndSerial <$> getObject)

data RecipientEncryptedKey = RecipientEncryptedKey { recipientEncryptedKeyIdentifier :: KeyAgreementRecipientIdentifier
...
instance ASN1Structure RecipientEncryptedKey where
    toASN1Fields RecipientEncryptedKey{..} = runPrintASN1State printer
        where printer = putObject recipientEncryptedKeyIdentifier
                        <> putObject recipientEncryptedKey
    fromASN1Fields = runParseASN1State parser
        where parser = RecipientEncryptedKey <$> getObject
                                             <*> getObject
...
instance ASN1Structure KeyAgreement where
    toASN1Fields KeyAgreement{..} = runPrintASN1State printer
        where printer = putObject keyAgreementVersion
    ...
    fromASN1Fields = runParseASN1State parser
        where parser = KeyAgreement <$> getObject
                                    <*> getExplicit 0
                                    <*> getExplicitMaybe 1
                                    <*> getObject
                                    <*> getSequenceOf
...
data RecipientKeyIdentifier = RecipientKeyIdentifier { recipientKeyIdentifier :: SubjectKeyIdentifier
...
instance ASN1Structure RecipientKeyIdentifier where
    toASN1Fields RecipientKeyIdentifier{..} = runPrintASN1State printer
        where printer = putObject recipientKeyIdentifier
                        <> putMaybe (putTime <$> recipientKeyTime)
                        <> putMaybe (putObject <$> recipientKeyAttribute)
    fromASN1Fields = runParseASN1State parser
        where parser = RecipientKeyIdentifier <$> getObject
...
instance ASN1Object a => ASN1Structure (OtherKeyAttribute a) where
    toASN1Fields OtherKeyAttribute{..} = runPrintASN1State printer
        where printer = putOID keyAttributeIdentifier
                        <> putObjectMaybe keyAttributeValue
    fromASN1Fields = runParseASN1State parser
        where parser = OtherKeyAttribute <$> getOID
                                         <*> getObjectMaybe
...
instance ASN1Structure KEKIdentifier where
    toASN1Fields KEKIdentifier{..} = runPrintASN1State printer
        where printer = putOctetString kekIdentifierValue
                        <> putMaybe (putTime <$> kekIdentifierTime)
                        <> putMaybe (putObject <$> kekIdentifierOther)
    fromASN1Fields = runParseASN1State parser
        where parser = KEKIdentifier <$> getOctetString
                                     <*> getNextMaybe toDateTime
                                     <*> getObjectMaybe
              toDateTime (ASN1Time _ t _) = Just t
              toDateTime _ = Nothing
...
instance ASN1Object EncryptedKey where
    toASN1 (EncryptedKey bs) = runPrintASN1State printer
        where printer = putOctetString bs
    fromASN1 = runParseASN1State parser
        where parser = EncryptedKey <$> getOctetString
...
instance ASN1Object Recipient where
    ...
    fromASN1 = runParseASN1State parser
        where parser = onContextMaybe 4 (RecipientOther <$> getOID <*> getObject)
                       `orChoice`
...
instance ASN1Structure EnvelopedData where
    toASN1Fields EnvelopedData{..} = runPrintASN1State printer
        where printer = putObject envelopedVersion
    ...
    fromASN1Fields = runParseASN1State parser
        where parser = EnvelopedData <$> getObject
                                     <*> getImplicitMaybe 0
                                     <*> getSetOf
                                     <*> getObject
                                     <*> (fmap unSetOf <$> getImplicitMaybe 1)
