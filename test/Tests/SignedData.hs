...
instance Monad m => Serial m ExtendedCertificate where
    series = decDepth $
        ExtendedCertificate <$> pure example
                            <~> pure example
                            <~> elements [ [], [ example ] ]
...
instance Monad m => Serial m RevocationChoice where
    series = cons1 RevocationCRL \/
        decDepth (RevocationOther <$> oidSeries <~> series)
...
sampleSignedData =
    BS.concat [ "MIIGlQYJKoZIhvcNAQcCoIIGhjCCBoICAQExCzAJBgUrDgMCGgUAMCsGCSqGSIb3DQEHAaAe"
...
testSignedData =
    testGroup "Data.Pkcs7.SignedData"
